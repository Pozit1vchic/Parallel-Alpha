...
- Кэширование загрузок в памяти (ModelCache)
"""
...
class ModelCache:
    ...
    Ключ: имя модели (str)
    Значение: torch.jit.ScriptModule или YOLO модель
    """
    ...
    def set(self, name: str, model: object) -> None:
    ...
    def clear_except(self, keep: list[str]) -> None:
        """Очистить кэш, оставив указанные модели."""
        with self._lock:
            to_remove = [k for k in self._cache if k not in keep]
            for k in to_remove:
                del self._cache[k]
...
class ModelManager:
    ...
    def prepare(
        ...
        Кэширование: модель загружается один раз и хранится в памяти.
        """
    ...
    def get_cached_model(self, name: str) -> Optional[object]:
        """Получить модель из кэша."""
        return self._cache.get(name)

    def set_cached_model(self, name: str, model: object) -> None:
        """Сохранить модель в кэш."""
        self._cache.set(name, model)

    def clear_cache(self) -> None:
        """Очистить кэш."""
        self._cache.clear()

    def clear_cache_except(self, keep: list[str]) -> None:
        """Очистить кэш, оставив указанные модели."""
        self._cache.clear_except(keep)